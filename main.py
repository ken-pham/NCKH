...
def init_parameters():
    ...
    # Setting
    parser.add_argument('--config', '-c', type=str, default='1008', help='ID of the using config', required=True)#ID của Sử dụng config
    parser.add_argument('--gpus', '-g', type=int, nargs='+', default=[], help='Using GPUs') #Sử dung GPU
    parser.add_argument('--seed', '-s', type=int, default=2, help='Random seed') # thoi gian nghi
    parser.add_argument('--debug', '-db', default=False, action='store_true', help='Debug')
    parser.add_argument('--pretrained_path', '-pp', type=str, default='pretrained\1008_resgcn-b19_ntu-xset120.pth.tar', help='Path to pretrained models')#Đường dẫn đến các mô hình được đào tạo trước
